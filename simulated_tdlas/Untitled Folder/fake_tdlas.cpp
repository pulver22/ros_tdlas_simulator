...
n.getParam("/simulated_tdlas/sensor0_hight",sensor0);
n.getParam("/simulated_tdlas/sensor1_hight",sensor1);
n.getParam("/simulated_tdlas/sensor2_hight",sensor2);
            
n.getParam("/simulated_tdlas/sensor0_model",robot_0_sensor_model);
n.getParam("/simulated_tdlas/sensor1_model",robot_1_sensor_model);
n.getParam("/simulated_tdlas/sensor2_model",robot_2_sensor_model);
  ...
  //flooring the position of sensor for measuring the concentration: value[int][int][int]
 /* int x_r0=int(amcl_pos.pose.pose.position.x);
  int y_r0=int(amcl_pos.pose.pose.position.y);
  int z_r0=int(amcl_pos.pose.pose.position.z);

  int x_r1=int(amcl_pos.pose.pose.position.x);
  int y_r1=int(amcl_pos.pose.pose.position.y);
  int z_r1=int(amcl_pos.pose.pose.position.z);

  int x_r2=int(amcl_pos.pose.pose.position.x);
  int y_r2=int(amcl_pos.pose.pose.position.y);
  int z_r2=int(amcl_pos.pose.pose.position.z);*/

  //Due to axis adjusment problem between rostage and rviz:
  /*int axis_xr0=x_r0;//+map_points[0];
  int axis_yr0=y_r0;//+map_points[1];
  int axis_zr0= z_r0;

  int axis_xr1=x_r1;//+map_points[2];
  int axis_yr1=y_r1;//+map_points[3];
  int axis_zr1= z_r1;

  int axis_xr2=x_r2;//+map_points[4];
  int axis_yr2=y_r2;//+map_points[5];
  int axis_zr2= z_r2;*/
  ...
  msgs_and_srvs::readings robot;
	


	//sensor response
	robot.r0_val=(sensor_response(servive_val[0], ch_id, 0,robot_0_sensor_model));
	robot.r1_val=(sensor_response(servive_val[1], ch_id, 1,robot_1_sensor_model));
	robot.r2_val=(sensor_response(servive_val[2], ch_id, 2,robot_2_sensor_model));


ROS_INFO("\nHEIGHTS OF SENSORS: %d  %d  %d %d %d %d\n" ,robot_0_sensor_model,robot_1_sensor_model,robot_2_sensor_model,sensor0,sensor1,sensor2);

	//concentration
	robot.r0_con=servive_val[0];
	robot.r1_con=servive_val[1];
	robot.r2_con=servive_val[2];

//writing log file:
/*  myfile <<t_stamp<<"  " <<ch_id<<"  "<<axis_xr0<<"  "<<axis_yr0<<"  "<<axis_zr0+sensor0<<"  "<<robot_0_sensor_model<<"  "<<servive_val[0] <<"  "<<robot.r0_val<<"  "<<axis_xr1<<"  "<<axis_yr1<<"  "<<axis_zr1+sensor1<<"  "<<robot_1_sensor_model<<"  "<<"  "<<servive_val[1]<<"  "<<robot.r1_val<<"  "<<axis_xr2<<"  "<<axis_yr2<<"  "<<axis_zr2+sensor2<<"  "<<robot_2_sensor_model<<"  "<<"  "<<servive_val[2] <<"  "<<robot.r2_val<<"\n";
*/
  t_stamp=t_stamp+0.5;
...
//sensor_0:
  ...
  connector[0].color.a =connector[1].color.a =connector[2].color.a = 1.0;
	...
	//closing file after writing data:
 	myfile.close(); 
}
